import { Geist, Geist_Mono, Lilita_One } from "next/font/google";
import "./globals.css";
import Link from "next/link";

const geistSans = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
});

const geistMono = Geist_Mono({
  variable: "--font-geist-mono",
  subsets: ["latin"],
});

const lilita = Lilita_One({
  variable: "--font-lilita",
  subsets: ["latin"],
  weight: "400",
});

export const metadata = {
  title: "Create Next App",
  description: "Generated by create next app",
};

export default function RootLayout({ children }) {
  return (
    <html lang="en">
      <body
        className={`${geistSans.variable} ${geistMono.variable} ${lilita.variable} antialiased flex justify-around`}
      >
        <div>
          <div>
            <Link href="/">
              <div className="flex cursor-pointer mt-3.5">
                <svg
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                  className="w-[30px] h-[30px] fill-white"
                >
                  <g>
                    <path d="M21.742 21.75l-7.563-11.179 7.056-8.321h-2.456l-5.691 6.714-4.54-6.714H2.359l7.29 10.776L2.25 21.75h2.456l6.035-7.118 4.818 7.118h6.191-.008zM7.739 3.818L18.81 20.182h-2.447L5.29 3.818h2.447z"></path>
                  </g>
                </svg>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <path d="M21.591 7.146L12.52 1.157c-.316-.21-.724-.21-1.04 0l-9.071 5.99c-.26.173-.409.456-.409.757v13.183c0 .502.418.913.929.913H9.14c.51 0 .929-.41.929-.913v-7.075h3.909v7.075c0 .502.417.913.928.913h6.165c.511 0 .929-.41.929-.913V7.904c0-.301-.158-.584-.408-.758z"></path>
                  </svg>
                </div>
                <div className="text-xl">Anasayfa</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/explore">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M10.25 3.75c-3.59 0-6.5 2.91-6.5 6.5s2.91 6.5 6.5 6.5c1.795 0 3.419-.726 4.596-1.904 1.178-1.177 1.904-2.801 1.904-4.596 0-3.59-2.91-6.5-6.5-6.5zm-8.5 6.5c0-4.694 3.806-8.5 8.5-8.5s8.5 3.806 8.5 8.5c0 1.986-.682 3.815-1.824 5.262l4.781 4.781-1.414 1.414-4.781-4.781c-1.447 1.142-3.276 1.824-5.262 1.824-4.694 0-8.5-3.806-8.5-8.5z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Keşfet</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/notifications">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M19.993 9.042C19.48 5.017 16.054 2 11.996 2s-7.49 3.021-7.999 7.051L2.866 18H7.1c.463 2.282 2.481 4 4.9 4s4.437-1.718 4.9-4h4.236l-1.143-8.958zM12 20c-1.306 0-2.417-.835-2.829-2h5.658c-.412 1.165-1.523 2-2.829 2zm-6.866-4l.847-6.698C6.364 6.272 8.941 4 11.996 4s5.627 2.268 6.013 5.295L18.864 16H5.134z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Bildirimler</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/messages">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M1.998 5.5c0-1.381 1.119-2.5 2.5-2.5h15c1.381 0 2.5 1.119 2.5 2.5v13c0 1.381-1.119 2.5-2.5 2.5h-15c-1.381 0-2.5-1.119-2.5-2.5v-13zm2.5-.5c-.276 0-.5.224-.5.5v2.764l8 3.638 8-3.636V5.5c0-.276-.224-.5-.5-.5h-15zm15.5 5.463l-8 3.636-8-3.638V18.5c0 .276.224.5.5.5h15c.276 0 .5-.224.5-.5v-8.037z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Mesajlar</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/grok">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 33 32"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M12.745 20.54l10.97-8.19c.539-.4 1.307-.244 1.564.38 1.349 3.288.746 7.241-1.938 9.955-2.683 2.714-6.417 3.31-9.83 1.954l-3.728 1.745c5.347 3.697 11.84 2.782 15.898-1.324 3.219-3.255 4.216-7.692 3.284-11.693l.008.009c-1.351-5.878.332-8.227 3.782-13.031L33 0l-4.54 4.59v-.014L12.743 20.544m-2.263 1.987c-3.837-3.707-3.175-9.446.1-12.755 2.42-2.449 6.388-3.448 9.852-1.979l3.72-1.737c-.67-.49-1.53-1.017-2.515-1.387-4.455-1.854-9.789-.931-13.41 2.728-3.483 3.523-4.579 8.94-2.697 13.561 1.405 3.454-.899 5.898-3.22 8.364C1.49 30.2.666 31.074 0 32l10.478-9.466"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Grok</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/lists">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                    width="26"
                    height="26"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="w-[26px] h-[26px]"
                  >
                    <rect x="3" y="3" width="18" height="18" rx="4" />
                    <line x1="8" y1="9" x2="16" y2="9" />
                    <line x1="8" y1="12" x2="16" y2="12" />
                  </svg>
                </div>
                <div className="text-xl">Listeler</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/bookmarks">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    width="26"
                    height="26"
                    className="w-[26px] h-[26px]"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M6 2h12a2 2 0 0 1 2 2v16l-8-5-8 5V4a2 2 0 0 1 2-2z" />
                  </svg>
                </div>
                <div className="text-xl">Yer İşaretleri</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/communuties">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M7.501 19.917L7.471 21H.472l.029-1.027c.184-6.618 3.736-8.977 7-8.977.963 0 1.95.212 2.87.672-.444.478-.851 1.03-1.212 1.656-.507-.204-1.054-.329-1.658-.329-2.767 0-4.57 2.223-4.938 6.004H7.56c-.023.302-.05.599-.059.917zm15.998.056L23.528 21H9.472l.029-1.027c.184-6.618 3.736-8.977 7-8.977s6.816 2.358 7 8.977zM21.437 19c-.367-3.781-2.17-6.004-4.938-6.004s-4.57 2.223-4.938 6.004h9.875zm-4.938-9c-.799 0-1.527-.279-2.116-.73-.836-.64-1.384-1.638-1.384-2.77 0-1.93 1.567-3.5 3.5-3.5s3.5 1.57 3.5 3.5c0 1.132-.548 2.13-1.384 2.77-.589.451-1.317.73-2.116.73zm-1.5-3.5c0 .827.673 1.5 1.5 1.5s1.5-.673 1.5-1.5-.673-1.5-1.5-1.5-1.5.673-1.5 1.5zM7.5 3C9.433 3 11 4.57 11 6.5S9.433 10 7.5 10 4 8.43 4 6.5 5.567 3 7.5 3zm0 2C6.673 5 6 5.673 6 6.5S6.673 8 7.5 8 9 7.327 9 6.5 8.327 5 7.5 5z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Topluluklar</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/premium">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M21.742 21.75l-7.563-11.179 7.056-8.321h-2.456l-5.691 6.714-4.54-6.714H2.359l7.29 10.776L2.25 21.75h2.456l6.035-7.118 4.818 7.118h6.191-.008zM7.739 3.818L18.81 20.182h-2.447L5.29 3.818h2.447z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Premium</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/profile">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M5.651 19h12.698c-.337-1.8-1.023-3.21-1.945-4.19C15.318 13.65 13.838 13 12 13s-3.317.65-4.404 1.81c-.922.98-1.608 2.39-1.945 4.19zm.486-5.56C7.627 11.85 9.648 11 12 11s4.373.85 5.863 2.44c1.477 1.58 2.366 3.8 2.632 6.46l.11 1.1H3.395l.11-1.1c.266-2.66 1.155-4.88 2.632-6.46zM12 4c-1.105 0-2 .9-2 2s.895 2 2 2 2-.9 2-2-.895-2-2-2zM8 6c0-2.21 1.791-4 4-4s4 1.79 4 4-1.791 4-4 4-4-1.79-4-4z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Profil</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/more">
              <div className="flex cursor-pointer mt-8.5">
                <div className="mr-4">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="w-[26px] h-[26px] fill-white"
                  >
                    <g>
                      <path d="M3.75 12c0-4.56 3.69-8.25 8.25-8.25s8.25 3.69 8.25 8.25-3.69 8.25-8.25 8.25S3.75 16.56 3.75 12zM12 1.75C6.34 1.75 1.75 6.34 1.75 12S6.34 22.25 12 22.25 22.25 17.66 22.25 12 17.66 1.75 12 1.75zm-4.75 11.5c.69 0 1.25-.56 1.25-1.25s-.56-1.25-1.25-1.25S6 11.31 6 12s.56 1.25 1.25 1.25zm9.5 0c.69 0 1.25-.56 1.25-1.25s-.56-1.25-1.25-1.25-1.25.56-1.25 1.25.56 1.25 1.25 1.25zM13.25 12c0 .69-.56 1.25-1.25 1.25s-1.25-.56-1.25-1.25.56-1.25 1.25-1.25 1.25.56 1.25 1.25z"></path>
                    </g>
                  </svg>
                </div>
                <div className="text-xl">Daha fazla</div>
              </div>
            </Link>
          </div>

          <div>
            <Link href="/post">
              <div className="flex px-14 py-4 bg-[#E7EBED] text-black font-bold rounded-full cursor-pointer mt-8.5">
                Gönderi yayınla
              </div>
            </Link>
          </div>

          <Link href="/profile">
            <div className="flex flex-col cursor-pointer mt-14">
              <p className="font-bold">Osman Salih Akbulut</p>
              <p className="text-sm text-gray-500">@osmansak58</p>
            </div>
          </Link>
        </div>

        <div className="fixed">{children}</div>

        <div>
          <div className="flex pl-2 pr-20 w-90 py-2 mt-1.5 border border-gray-700 rounded-full">
            <label className="mt-1" htmlFor="xsearch">
              <svg viewBox="0 0 24 24" width="28" height="16" fill="gray">
                <path d="M10.25 3.75c-3.59 0-6.5 2.91-6.5 6.5s2.91 6.5 6.5 6.5c1.795 0 3.419-.726 4.596-1.904 1.178-1.177 1.904-2.801 1.904-4.596 0-3.59-2.91-6.5-6.5-6.5zm-8.5 6.5c0-4.694 3.806-8.5 8.5-8.5s8.5 3.806 8.5 8.5c0 1.986-.682 3.815-1.824 5.262l4.781 4.781-1.414 1.414-4.781-4.781c-1.447 1.142-3.276 1.824-5.262 1.824-4.694 0-8.5-3.806-8.5-8.5z"></path>
              </svg>
            </label>
            <input
              type="text"
              id="xsearch"
              name="xsearch"
              placeholder="Search"
            ></input>
          </div>

          <div className="flex flex-col pl-4 py-2 border border-gray-700 rounded-2xl mt-4">
            <p className="font-bold text-xl mt-1">You might like</p>
            <div className="mt-5 flex cursor-pointer">
              <div>
                <p className="font-bold">Mertbaba Okulmuş</p>
                <p className="text-sm text-gray-500">@okulmus_mert</p>
              </div>
              <div className="px-4 h-8 ml-26 bg-white text-black rounded-full font-bold text-sm flex items-center">
                Follow
              </div>
            </div>
            <div className="mt-4 flex cursor-pointer">
              <div>
                <p className="font-bold">Osman Salih Akbulut</p>
                <p className="text-sm text-gray-500">@osmansak58</p>
              </div>
              <div className="px-4 h-8 ml-22 bg-white text-black rounded-full font-bold text-sm flex items-center">
                Follow
              </div>
            </div>
            <div className="mt-4 flex cursor-pointer">
              <div>
                <p className="font-bold">Galatasaray SK</p>
                <p className="text-sm text-gray-500">@GalatasaraySK</p>
              </div>
              <div className="px-4 h-8 ml-32.5 bg-white text-black rounded-full font-bold text-sm flex items-center">
                Follow
              </div>
            </div>
            <p className="text-sm text-sky-600 mt-5 pb-2 cursor-pointer">
              Show more
            </p>
          </div>

          <div className="flex flex-col pl-4 py-2 border border-gray-700 rounded-2xl mt-4">
            <p className="font-bold text-xl mt-1">What's happening</p>
            <div className="mt-5 flex cursor-pointer">
              <div>
                <p className="text-sm text-gray-500">Trending in Turkiye</p>
                <p className="font-bold">Papa</p>
                <p className="text-sm text-gray-500">272K posts</p>
              </div>
            </div>
            <div className="mt-5 flex cursor-pointer">
              <div>
                <p className="text-sm text-gray-500">Trending in Turkiye</p>
                <p className="font-bold">Şerif</p>
                <p className="text-sm text-gray-500">5,490 posts</p>
              </div>
            </div>
            <div className="mt-5 flex cursor-pointer">
              <div>
                <p className="text-sm text-gray-500">Trending in Turkiye</p>
                <p className="font-bold">live</p>
                <p className="text-sm text-gray-500">1,040 posts</p>
              </div>
            </div>

            <Link href="/explore">
              <p className="text-sm text-sky-600 mt-5 pb-2 cursor-pointer">
                Show more
              </p>
            </Link>
          </div>
        </div>
      </body>
    </html>
  );
}
